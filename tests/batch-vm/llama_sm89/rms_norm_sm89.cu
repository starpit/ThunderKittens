    ...
    struct storer {
        static __device__ void run(const Globals &g, state<Config> &s) {
            ...
            if (warp::laneid() == 0) {
                ...
                // group::store is synchronous: all threads in warp participate.
